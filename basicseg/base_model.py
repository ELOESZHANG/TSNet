...
from basicseg.warmup_scheduler import GradualWarmupScheduler
...
class Base_model():
    ...
    def setup_loss(self):
        ...
        for loss in loss_opt:
            self.loss_fn[loss_opt[loss]['type']] = build_loss(loss_opt[loss])
    ...
    def setup_lr_schduler(self):
        ...
        if scheduler_type is None:
            ...
            if scheduler_type in ['MultiStepLR', 'MultiStepRestartLR']:
                ...
                # warmup_epochs = 10
                # scheduler_cosine = torch.optim.lr_scheduler.CosineAnnealingLR(self.optim, self.T_max- warmup_epochs,
                #                                                                **lr_opt['scheduler'])
                # self.scheduler = GradualWarmupScheduler(self.optim, multiplier=1, total_epoch=warmup_epochs,
                #                                    after_scheduler=scheduler_cosine)
                # print(self.optim,self.T_max,lr_opt['scheduler'])
                self.scheduler = \
    ...
    def update_learning_rate(self, current_iter, idx):
        ...
        if current_iter < self.warmup_iter:
            ...
            warmup_lr = self.init_lr / self.warmup_iter * current_iter
            # set learning rate
    ...
    def load_network(self, net, load_path, strict=True, param_key=None, **kwargs):
        ...
        if param_key is not None:
            ...
            # load_net = {k: load_net[param_key] for k, _ in net.state_dict()}
    ...
    def reduce_dict(self, inp_dict, reduction='mean'):
        ...
        Args:
            inp_dict (OrderedDict): dict to be reduced in all ranks.
